...
def plotly_CitySelection(cities, plot_info, pred_traffic, plotly_save_path):
    ...
    shutil.rmtree(f"{plotly_save_path}/", ignore_errors = True)
    os.mkdir(f"{plotly_save_path}/")
    ...
    for city in cities:
        ...
        for city_col_name, col_info in plot_info:
            ...
            if(city_col_name.startswith(city)):
                ...
                if(behavior != 'constant'):
                    subplot_names[cols.index(col_name)] = f"{factors[cols.index(col_name)]} factors {behavior} every {round(np.abs(np.log(2) / col_info['growth'] * col_info['duration']), 1)} years"
                else:
                    subplot_names[cols.index(col_name)] = f"{factors[cols.index(col_name)]} factors expected to remain constant"
        ...
        fig2.update_layout(
            title_text = f"Growths in various macro-economic factors of {city}",
        ...
        with open(f'{plotly_save_path}/{city}_CitySelection_Graph1.txt', 'w') as save_file:
            save_file.write(pyo.plot(fig, output_type = 'div', include_plotlyjs = False))
        with open(f'{plotly_save_path}/{city}_CitySelection_Graph2.txt', 'w') as save_file:
            save_file.write(pyo.plot(fig2, output_type = 'div', include_plotlyjs = False))
